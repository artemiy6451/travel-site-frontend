import type { Excursion, ExcursionCreate, ExcursionUpdate, ExcursionFullInfo, ExcursionDetails, ExcursionDetailsCreate, ExcursionDetailsUpdate, ItineraryItem } from "@/types/excursion";
...
class Api {
  ...
  async searchExcursions(query: string): Promise<Excursion[]> {
    ...
    const response = await this.request<Excursion[]>(`/excursions/search/?q=${encodeURIComponent(query)}`);
    cache.set(cacheKey, response, 2 * 60 * 1000);
    return response;
  }
  ...
  async createExcursionDetails(excursionId: number, details: ExcursionDetailsCreate): Promise<ExcursionDetails> {
    const response = await this.request<ExcursionDetails>(`/excursions/${excursionId}/details`, {
      method: 'POST',
      headers: {
        ...this.getAuthHeaders(),
  ...
  async updateExcursionDetails(excursionId: number, details: ExcursionDetailsUpdate): Promise<ExcursionDetails> {
    const response = await this.request<ExcursionDetails>(`/excursions/${excursionId}/details`, {
      method: 'PUT',
      headers: {
        ...this.getAuthHeaders(),
  ...
  async createOrUpdateExcursionDetails(excursionId: number, details: ExcursionDetailsCreate): Promise<ExcursionDetails> {
    const response = await this.request<ExcursionDetails>(`/excursions/${excursionId}/details`, {
      method: 'PATCH',
      headers: {
        ...this.getAuthHeaders(),
